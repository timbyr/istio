...
func validateServerPort(port *networking.Port) (errs error) {
	...
	if protocol.Parse(port.Protocol) == protocol.Unsupported {
		errs = appendErrors(errs, fmt.Errorf("invalid protocol %q, supported protocols are HTTP, HTTP2, GRPC, GRPC-WEB, MONGO, REDIS, MYSQL, KAFKA, ZOOKEEPER, TCP", port.Protocol))
	}
	if port.Number > 0 {
